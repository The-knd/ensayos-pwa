...
export function CreditStudyPage() {
  ...
  const [verdict, setVerdict] = useState<{ creditId: string; mockup: Mockup } | null>(null);
  ...
  return (
    <div className="s2">
      <AppBar title="Crédito Impulsa Ferretero" />
      <div className="body">
        {verdict ? (
          <div style={{ padding: '10px 2px' }}>
            {verdict.mockup.decision === 'approved' ? (
              <>
                <div className="addr-result" style={{ background: 'var(--green-soft)', color: 'var(--green-deep)', borderColor: 'rgba(62,155,97,0.2)' }}>
                  ...
                  <div>
                    <strong style={{ fontSize: 14 }}>¡Aprobado!</strong>
                    <div style={{ fontSize: 12, marginTop: 4 }}>
                      Cupo aprobado: <strong>${verdict.mockup.approvedLimit?.toLocaleString('es-CO')}</strong>
                    </div>
                    <div style={{ fontSize: 11, marginTop: 4, opacity: 0.9 }}>{verdict.mockup.reason}</div>
                  </div>
                </div>
                <div className="rowbtn">
                  <button className="btn btn-ghost" onClick={() => navigate('/credits')} disabled={submitting}>Volver</button>
                  <button className="btn btn-primary" onClick={goToSign} disabled={submitting} style={{ opacity: submitting ? 0.6 : 1 }}>
                    {submitting ? 'Procesando…' : 'Continuar a firma'}
                  </button>
                </div>
              </>
            ) : (
              <>
                <div className="note">
                  ...
                  <div>
                    <strong style={{ fontSize: 14 }}>Solicitud no aprobada</strong>
                    <div style={{ fontSize: 12, marginTop: 4 }}>{verdict.mockup.reason}</div>
                  </div>
                </div>
                <div className="rowbtn">
                  <button className="btn btn-ghost" onClick={() => setVerdict(null)}>Reintentar</button>
                  <button className="btn btn-primary" onClick={() => navigate('/credits')}>Volver a créditos</button>
                </div>
              </>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="note">
              ...
              <p>
                ...
                acepta el tratamiento de datos para iniciar el estudio del crédito Impulsa Ferretero.
              </p>
            </div>
